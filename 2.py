...
bg_image_raw = pygame.image.load("image.png")  # updated to png as requested
# no scaling! keep original tall image
...
class Game:
    ...
    def reset(self):
        self.blocks = []
        self.offset_y = bg_image_raw.get_height() - HEIGHT  # Start showing bottom of bg
        ...
        self.current_block = Block(
            x=0, y=HEIGHT - BLOCK_HEIGHT * 2 + self.offset_y, width=WIDTH // 3,
            direction=self.current_direction, color=COLORS[color_index]
        )

        base_block = Block(
            WIDTH // 2 - WIDTH // 6, HEIGHT - BLOCK_HEIGHT + self.offset_y,
    ...
    def drop_block(self):
        ...
        new_y = self.current_block.y - BLOCK_HEIGHT  # blocks go up logically (y decreases)

        # **Fix here: increase offset_y so the view scrolls DOWN, making blocks appear to go down**
        # offset_y cannot exceed background height - screen height (top of image)
        if self.offset_y > 0 and (HEIGHT - (new_y - self.offset_y)) >= MAX_VISIBLE_Y:
            self.offset_y -= BLOCK_HEIGHT  # scroll camera DOWN by decreasing offset_y
    ...
    def draw(self, surface):
        ...
        # Calculate offset for bg image (scrolls up with blocks)
        offset = self.offset_y

        # Make sure offset is within bounds
        offset = max(0, min(offset, bg_image_raw.get_height() - HEIGHT))

        visible_rect = pygame.Rect(0, offset, WIDTH, HEIGHT)
        surface.blit(bg_image_raw.subsurface(visible_rect), (0, 0))

        # Fill with green if background scrolled off screen completely (shouldn't happen with limits)
        if offset <= 0:
            green_height = abs(offset)
            pygame.draw.rect(surface, BG_FILL_COLOR, (0, HEIGHT - green_height, WIDTH, green_height))
